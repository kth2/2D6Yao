...
```
用户起卦
  → 引擎算出完整卦盘（纳甲/六亲/世应/旺衰/空破/伏神，确定性可推）
  → 取用神（用神·原神·忌神·仇神，逐爻列出支持与反证）
  → 规则匹配（rules.jsonl，命中条文）
  → 检索相似书例（by_gua + by_tag + 向量）
  → LLM 在以上证据之上推理，逐条标注依据
```

**事实由引擎定，推理由 LLM 做，每步标注依据。**

分工的界线在「算得出来的」和「要判断的」之间：

| 环节 | 谁来做 | 为什么 |
|---|---|---|
| 纳甲、六亲、世应、旺衰、旬空、月破日破、伏神、动变 | 引擎 | 确定性可推；LLM 凭记忆复述这些必错，且错得看不出来 |
| 用神取舍、条文取舍、强弱权衡、应期推断 | LLM | 条文彼此常相矛盾（旺与衰、空与不空各有说法），落到具体卦上要判断 |
| 断语措辞 | LLM | — |

两条硬约束：

1. **盘面事实是既定输入**，LLM 只能在其上推理，不得重算或改写；
2. **每句结论标明出处** —— 是「依据条文 [一-一1-9]」「参照书例 #12」，还是「推断」。

这样仍然可追可查：断错了能定位到是哪条依据不对，还是哪一步推理不对。
原则从「LLM 不做推理」放宽到「LLM 的推理必须带依据」，
是因为检索只能给出通则，通则落到单卦上必须有人判断 —— 这一步正是断卦本身。
